...
import { SocialesGradoService } from 'src/app/core/services/common/grado/sociales-grado.service';
...
import { SocialesService } from 'src/app/core/services/common/original/sociales.service';
import { TimeFinalService } from 'src/app/core/services/common/original/time-final.service';


@Component({
  selector: 'app-sociales-niveles',
  templateUrl: './sociales-niveles-grado.component.html',
  styleUrls: ['./sociales-niveles-grado.component.scss']
})
export class SocialesNivelesGradoComponent {
  ...
  $loading: Signal<boolean> = this.socialesGradoService.$loading;
  ...
  constructor(private socialesGradoService: SocialesGradoService,
              private progressSocialesGradoService: DataProgressGradoService,
              private finalTimeSocialesGradoService: TimeFinalGradoService,
              private resetGameButtonSocialesGradoService: ResetGameButtonGradoService,
              private route: ActivatedRoute,
              private router: Router,
              private levelSocialesGradoService: LevelGradoService
            ) {
              this.loadTimerState();
              }
  ...
  ngOnInit(): void {
    ...
    this.socialesGradoService.getGradoSheets().subscribe(data => {
      if (data) {
  ...
  loadTimerState() {
    const storedClock = localStorage.getItem('clockSocialesGrado');
    const storedFinalTime = localStorage.getItem('finalTimeSocialesGrado');
    ...
    this.resetGameButtonSocialesGradoService.resetGameSociales$.subscribe(() => {
      this.resetGame();
    });

    if (storedFinalTime && allLevelsCompleted) {
      this.clock = parseInt(storedFinalTime, 10);
      this.finalTimeSocialesGradoService.setFinalTimeSociales(this.clock);
      } else if (storedClock) {
  ...
  if (this.allLevelsCompleted()) {
    ...
    localStorage.setItem('finalTimeSocialesGrado', this.finalTime.toString());
    // SOLUCION DEL TIMER QUE LLEGABA EN CERO
    this.finalTimeSocialesGradoService.getFinalTimeSociales().next(this.finalTime);
    }
  }
  ...
  nextQuestion() {
    if (this.currentQuestionIndex < this.levels[this.currentLevelIndex].length - 1) {
      ...
      if (this.allQuestionsAnswered()) {
        ...
        this.resetGameButtonSocialesGradoService.setShowResetButtonSociales(true);
        this.stopTimer();
        this.storeFinalTime();
      }
    }
  ...
  onOptionSelected(levelIndex: number, questionIndex: number, option: string) {
    ...
    this.progressSocialesGradoService.recordAnswerSociales(isCorrect);
  ...
  startTimer() {
    this.stopTimer();
    const storedClock = parseInt(localStorage.getItem('clockSocialesGrado') || '0', 10);
    this.clock = storedClock;
    this.startTime = Date.now();
    localStorage.setItem('startTimeSocialesGrado', this.startTime.toString());
    this.timerSubscription = timer(0, 1000).subscribe(() => {
      const currentTime = Date.now();
      this.clock = storedClock + Math.floor((currentTime - this.startTime) / 1000);
      localStorage.setItem('clockSocialesGrado', this.clock.toString());
    });
  }
  ...
  stopTimer() {
    ...
    localStorage.setItem('clockSocialesGrado', this.clock.toString());
    localStorage.removeItem('startTimeSocialesGrado');
  }
  ...
  resetGame() {
    ...
    if(this.allLevelsCompleted()) {
      return this.resetGameButtonSocialesGradoService.setShowResetButtonSociales(true);
    }
  ...
  resetLevel(levelIndex: number) {
    ...
    this.progressSocialesGradoService.removeAnswersForLevelSociales(levelIndex);
  ...
  isLevelCompleted(levelIndex: number): boolean {
    ...
    this.levelSocialesGradoService.levelCompletedSocialesService(levelIndex);
    return true;
  }
  ...
  private checkAllLevelsCompleted() {
    const allLevelsCompleted = this.allLevelsCompleted();
    this.resetGameButtonSocialesGradoService.setShowResetButtonSociales(allLevelsCompleted);
  ...
  saveSelectedOptions() {
    localStorage.setItem('selectedOptionsSocialesGrado', JSON.stringify(this.selectedOptions));
  }

  // cargamos las preguntas que seleccionamos
  loadSelectedOptions() {
    const storedOptions = localStorage.getItem('selectedOptionsSocialesGrado');
    if (storedOptions) {
  ...
  private restoreSelections() {
    const storedSelections = localStorage.getItem('selectedOptionsSocialesGrado');
    if (storedSelections) {
