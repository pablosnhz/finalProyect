...
import { SheetsDatesService } from 'src/app/core/services/common/original/sheets-dates.service';
...
import { SheetsDatesGradoService } from 'src/app/core/services/common/grado/sheets-grado-dates.service';
...
templateUrl: './niveles-matematica-grado.component.html',
styleUrls: ['./niveles-matematica-grado.component.scss']
})
export class NivelesMatematicaGradoComponent implements OnInit, OnDestroy {
...
$loading: Signal<boolean> = this.sheetsGradoService.$loading;
...
constructor(private sheetsGradoService: SheetsDatesGradoService,
            private progressGradoService: DataProgressGradoService,
            private finalTimeGradoService: TimeFinalGradoService,
            private resetGameButtonGradoService: ResetGameButtonGradoService,
...
ngOnInit(): void {
  ...
  this.sheetsGradoService.getGradoSheets().subscribe(data => {
    if (data) {
...
loadTimerState() {
  const storedClock = localStorage.getItem('clockGrado');
  const storedFinalTime = localStorage.getItem('finalTimeGrado');
  ...
  this.resetGameButtonGradoService.resetGame$.subscribe(() => {
    this.resetGame();
  });

  if (storedFinalTime && allLevelsCompleted) {
    this.clock = parseInt(storedFinalTime, 10);
    this.finalTimeGradoService.setFinalTime(this.clock);
    } else if (storedClock) {
...
if (this.allLevelsCompleted()) {
  ...
  localStorage.setItem('finalTimeGrado', this.finalTime.toString());
  // SOLUCION DEL TIMER QUE LLEGABA EN CERO
  this.finalTimeGradoService.getFinalTime().next(this.finalTime);
  }
}
...
nextQuestion() {
  if (this.currentQuestionIndex < this.levels[this.currentLevelIndex].length - 1) {
    ...
    if (this.allQuestionsAnswered()) {
      ...
      this.resetGameButtonGradoService.setShowResetButton(true);
      this.stopTimer();
      this.storeFinalTime();
    }
  }
...
onOptionSelected(levelIndex: number, questionIndex: number, option: string) {
  ...
  this.progressGradoService.recordAnswer(isCorrect);
...
startTimer() {
  this.stopTimer();
  const storedClock = parseInt(localStorage.getItem('clockGrado') || '0', 10);
  this.clock = storedClock;
  this.startTime = Date.now();
  localStorage.setItem('startTimeGrado', this.startTime.toString());
  this.timerSubscription = timer(0, 1000).subscribe(() => {
    const currentTime = Date.now();
    this.clock = storedClock + Math.floor((currentTime - this.startTime) / 1000);
    localStorage.setItem('clockGrado', this.clock.toString());
  });
}
...
stopTimer() {
  ...
  localStorage.setItem('clockGrado', this.clock.toString());
  localStorage.removeItem('startTimeGrado');
}
...
resetGame() {
  ...
  if(this.allLevelsCompleted()) {
    return this.resetGameButtonGradoService.setShowResetButton(true);
  }
...
resetLevel(levelIndex: number) {
  ...
  this.progressGradoService.removeAnswersForLevel(levelIndex);
...
isLevelCompleted(levelIndex: number): boolean {
  ...
  this.levelGradoService.levelCompletedService(levelIndex);
  return true;
}
...
private checkAllLevelsCompleted() {
  const allLevelsCompleted = this.allLevelsCompleted();
  this.resetGameButtonGradoService.setShowResetButton(allLevelsCompleted);
...
saveSelectedOptions() {
  localStorage.setItem('selectedOptionsGrado', JSON.stringify(this.selectedOptions));
}

// cargamos las preguntas que seleccionamos
loadSelectedOptions() {
  const storedOptions = localStorage.getItem('selectedOptionsGrado');
  if (storedOptions) {
...
private restoreSelections() {
  const storedSelections = localStorage.getItem('selectedOptionsGrado');
  if (storedSelections) {
