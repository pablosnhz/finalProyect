...
import { InglesGradoService } from 'src/app/core/services/common/grado/ingles-grado.service';
...
@Component({
  selector: 'app-ingles-niveles',
  templateUrl: './ingles-niveles-grado.component.html',
  styleUrls: ['./ingles-niveles-grado.component.scss']
})
export class InglesNivelesGradoComponent {
...
$loading: Signal<boolean> = this.inglesGradoService.$loading;
...
constructor(private inglesGradoService: InglesGradoService,
            private progressInglesGradoService: DataProgressGradoService,
            private finalTimeInglesGradoService: TimeFinalGradoService,
            private resetGameButtonInglesGradoService: ResetGameButtonGradoService,
            private route: ActivatedRoute,
            private router: Router,
            private levelInglesGradoService: LevelGradoService
          ) {
            this.loadTimerState();
            }
...
ngOnInit(): void {
  ...
  this.inglesGradoService.getGradoSheets().subscribe(data => {
    if (data) {
...
loadTimerState() {
  const storedClock = localStorage.getItem('clockInglesGrado');
  const storedFinalTime = localStorage.getItem('finalTimeInglesGrado');
  ...
  this.resetGameButtonInglesGradoService.resetGameIngles$.subscribe(() => {
    this.resetGame();
  });

  if (storedFinalTime && allLevelsCompleted) {
    this.clock = parseInt(storedFinalTime, 10);
    this.finalTimeInglesGradoService.setFinalTimeIngles(this.clock);
    } else if (storedClock) {
...
if (this.allLevelsCompleted()) {
  ...
  localStorage.setItem('finalTimeInglesGrado', this.finalTime.toString());
  // SOLUCION DEL TIMER QUE LLEGABA EN CERO
  this.finalTimeInglesGradoService.getFinalTimeIngles().next(this.finalTime);
  }
}
...
nextQuestion() {
  if (this.currentQuestionIndex < this.levels[this.currentLevelIndex].length - 1) {
    ...
    if (this.allQuestionsAnswered()) {
      ...
      this.resetGameButtonInglesGradoService.setShowResetButtonIngles(true);
      this.stopTimer();
      this.storeFinalTime();
    }
  }
...
onOptionSelected(levelIndex: number, questionIndex: number, option: string) {
  ...
  this.progressInglesGradoService.recordAnswerIngles(isCorrect);
...
startTimer() {
  this.stopTimer();
  const storedClock = parseInt(localStorage.getItem('clockInglesGrado') || '0', 10);
  this.clock = storedClock;
  this.startTime = Date.now();
  localStorage.setItem('startTimeInglesGrado', this.startTime.toString());
  this.timerSubscription = timer(0, 1000).subscribe(() => {
    const currentTime = Date.now();
    this.clock = storedClock + Math.floor((currentTime - this.startTime) / 1000);
    localStorage.setItem('clockInglesGrado', this.clock.toString());
  });
}
...
stopTimer() {
  ...
  localStorage.setItem('clockInglesGrado', this.clock.toString());
  localStorage.removeItem('startTimeInglesGrado');
}
...
resetGame() {
  ...
  if(this.allLevelsCompleted()) {
    return this.resetGameButtonInglesGradoService.setShowResetButtonIngles(true);
  }
...
resetLevel(levelIndex: number) {
  ...
  this.progressInglesGradoService.removeAnswersForLevelIngles(levelIndex);
...
isLevelCompleted(levelIndex: number): boolean {
  ...
  this.levelInglesGradoService.levelCompletedInglesService(levelIndex);
  return true;
}
...
private checkAllLevelsCompleted() {
  const allLevelsCompleted = this.allLevelsCompleted();
  this.resetGameButtonInglesGradoService.setShowResetButtonIngles(allLevelsCompleted);
...
saveSelectedOptions() {
  localStorage.setItem('selectedOptionsInglesGrado', JSON.stringify(this.selectedOptions));
}

// cargamos las preguntas que seleccionamos
loadSelectedOptions() {
  const storedOptions = localStorage.getItem('selectedOptionsInglesGrado');
  if (storedOptions) {
...
private restoreSelections() {
  const storedSelections = localStorage.getItem('selectedOptionsInglesGrado');
  if (storedSelections) {
