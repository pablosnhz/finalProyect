...
import { LecturaCriticaGradoService } from 'src/app/core/services/common/grado/lectura-grado-critica.service';
...
import { LecturaCriticaService } from 'src/app/core/services/common/original/lectura-critica.service';
...
@Component({
  ...
  templateUrl: './lectura-niveles-grado.component.html',
  styleUrls: ['./lectura-niveles-grado.component.scss']
})
export class LecturaNivelesGradoComponent {
...
$loading: Signal<boolean> = this.lecturaGradoService.$loading;
...
constructor(private lecturaGradoService: LecturaCriticaGradoService,
            private progressLecturaGradoService: DataProgressGradoService,
            private finalTimeLecturaGradoService: TimeFinalGradoService,
            private resetGameButtonLecturaGradoService: ResetGameButtonGradoService,
            private route: ActivatedRoute,
            private router: Router,
            private levelLecturaGradoService: LevelGradoService
          ) {
            this.loadTimerState();
            }
...
ngOnInit(): void {
  ...
  this.lecturaGradoService.getGradoSheets().subscribe(data => {
    if (data) {
...
loadTimerState() {
  const storedClock = localStorage.getItem('clockLecturaGrado');
  const storedFinalTime = localStorage.getItem('finalTimeLecturaGrado');
  ...
  this.resetGameButtonLecturaGradoService.resetGameLectura$.subscribe(() => {
    this.resetGame();
  });

  if (storedFinalTime && allLevelsCompleted) {
    this.clock = parseInt(storedFinalTime, 10);
    this.finalTimeLecturaGradoService.setFinalTimeLectura(this.clock);
    } else if (storedClock) {
...
if (this.allLevelsCompleted()) {
  ...
  localStorage.setItem('finalTimeLecturaGrado', this.finalTime.toString());
  // SOLUCION DEL TIMER QUE LLEGABA EN CERO
  this.finalTimeLecturaGradoService.getFinalTimeLectura().next(this.finalTime);
  }
}
...
nextQuestion() {
  if (this.currentQuestionIndex < this.levels[this.currentLevelIndex].length - 1) {
    ...
    if (this.allQuestionsAnswered()) {
      ...
      this.resetGameButtonLecturaGradoService.setShowResetButtonLectura(true);
      this.stopTimer();
      this.storeFinalTime();
    }
  }
...
onOptionSelected(levelIndex: number, questionIndex: number, option: string) {
  ...
  this.progressLecturaGradoService.recordAnswerLectura(isCorrect);
...
startTimer() {
  this.stopTimer();
  const storedClock = parseInt(localStorage.getItem('clockLecturaGrado') || '0', 10);
  this.clock = storedClock;
  this.startTime = Date.now();
  localStorage.setItem('startTimeLecturaGrado', this.startTime.toString());
  this.timerSubscription = timer(0, 1000).subscribe(() => {
    const currentTime = Date.now();
    this.clock = storedClock + Math.floor((currentTime - this.startTime) / 1000);
    localStorage.setItem('clockLecturaGrado', this.clock.toString());
  });
}
...
stopTimer() {
  ...
  localStorage.setItem('clockLecturaGrado', this.clock.toString());
  localStorage.removeItem('startTimeLecturaGrado');
}
...
resetGame() {
  ...
  if(this.allLevelsCompleted()) {
    return this.resetGameButtonLecturaGradoService.setShowResetButtonLectura(true);
  }
...
resetLevel(levelIndex: number) {
  ...
  const lockButton = document.getElementById('levelLockButtonGrado' + (levelIndex + 1)) as HTMLElement;
  if (lockButton) {
  ...
  this.progressLecturaGradoService.removeAnswersForLevelLectura(levelIndex);
...
isLevelCompleted(levelIndex: number): boolean {
  ...
  this.levelLecturaGradoService.levelCompletedLecturaService(levelIndex);
  return true;
}
...
private checkAllLevelsCompleted() {
  const allLevelsCompleted = this.allLevelsCompleted();
  this.resetGameButtonLecturaGradoService.setShowResetButtonLectura(allLevelsCompleted);
...
saveSelectedOptions() {
  localStorage.setItem('selectedOptionsLecturaGrado', JSON.stringify(this.selectedOptions));
}

// cargamos las preguntas que seleccionamos
loadSelectedOptions() {
  const storedOptions = localStorage.getItem('selectedOptionsLecturaGrado');
  if (storedOptions) {
...
private restoreSelections() {
  const storedSelections = localStorage.getItem('selectedOptionsLecturaGrado');
  if (storedSelections) {
