...
constructor(private lecturaService: LecturaCriticaService,
            private progressLecturaService: DataProgressService,
            private finalTimeLecturaService: TimeFinalService,
            private resetGameButtonLecturaService: ResetGameButtonService,
            private route: ActivatedRoute,
            private router: Router,
            private levelLecturaService: LevelService
          ) {
            this.loadTimerState();
            }
...
loadTimerState() {
  ...
  this.resetGameButtonLecturaService.resetGameLectura$.subscribe(() => {
    this.resetGame();
  });

  if (storedFinalTime && allLevelsCompleted) {
    this.clock = parseInt(storedFinalTime, 10);
    this.finalTimeLecturaService.setFinalTimeLectura(this.clock);
    } else if (storedClock) {
...
if (this.allLevelsCompleted()) {
  ...
  localStorage.setItem('finalTimeLectura', this.finalTime.toString());
  // SOLUCION DEL TIMER QUE LLEGABA EN CERO
  this.finalTimeLecturaService.getFinalTimeLectura().next(this.finalTime);
  }
}
...
nextQuestion() {
  if (this.currentQuestionIndex < this.levels[this.currentLevelIndex].length - 1) {
    ...
    if (this.allQuestionsAnswered()) {
      ...
      this.resetGameButtonLecturaService.setShowResetButtonLectura(true);
      this.stopTimer();
      this.storeFinalTime();
    }
  }
...
onOptionSelected(levelIndex: number, questionIndex: number, option: string) {
  ...
  this.progressLecturaService.recordAnswerLectura(isCorrect);
...
startTimer() {
  this.stopTimer();
  const storedClock = parseInt(localStorage.getItem('clockLectura') || '0', 10);
  this.clock = storedClock;
  this.startTime = Date.now();
  localStorage.setItem('startTimeLectura', this.startTime.toString());
  this.timerSubscription = timer(0, 1000).subscribe(() => {
    const currentTime = Date.now();
    this.clock = storedClock + Math.floor((currentTime - this.startTime) / 1000);
    localStorage.setItem('clockLectura', this.clock.toString());
  });
}
...
stopTimer() {
  ...
  localStorage.setItem('clockLectura', this.clock.toString());
  localStorage.removeItem('startTimeLectura');
}
...
resetGame() {
  ...
  if(this.allLevelsCompleted()) {
    return this.resetGameButtonLecturaService.setShowResetButtonLectura(true);
  }
...
resetLevel(levelIndex: number) {
  ...
  this.progressLecturaService.removeAnswersForLevelLectura(levelIndex);
...
isLevelCompleted(levelIndex: number): boolean {
  ...
  this.levelLecturaService.levelCompletedLecturaService(levelIndex);
  return true;
}
...
private checkAllLevelsCompleted() {
  const allLevelsCompleted = this.allLevelsCompleted();
  this.resetGameButtonLecturaService.setShowResetButtonLectura(allLevelsCompleted);
...
saveSelectedOptions() {
  localStorage.setItem('selectedOptionsLectura', JSON.stringify(this.selectedOptions));
}

// cargamos las preguntas que seleccionamos
loadSelectedOptions() {
  const storedOptions = localStorage.getItem('selectedOptionsLectura');
  if (storedOptions) {
...
private restoreSelections() {
  const storedSelections = localStorage.getItem('selectedOptionsLectura');
  if (storedSelections) {
