...
@Component({
  ...
  templateUrl: './sociales-niveles.component.html',
  styleUrls: ['./sociales-niveles.component.scss']
})
export class SocialesNivelesComponent {
  ...
  constructor(private socialesService: SocialesService,
              ...
              private levelSocialesService: LevelService
            ) {
              this.loadTimerState();
              }
  ...
  loadTimerState() {
    const storedClock = localStorage.getItem('clockSociales');
    const storedFinalTime = localStorage.getItem('finalTimeSociales');
    ...
    this.resetGameButtonSocialesService.resetGameSociales$.subscribe(() => {
      this.resetGame();
    });

    if (storedFinalTime && allLevelsCompleted) {
      this.clock = parseInt(storedFinalTime, 10);
      this.finalTimeSocialesService.setFinalTimeSociales(this.clock);
      } else if (storedClock) {
  ...
  if (this.allLevelsCompleted()) {
    ...
    localStorage.setItem('finalTimeSociales', this.finalTime.toString());
    // SOLUCION DEL TIMER QUE LLEGABA EN CERO
    this.finalTimeSocialesService.getFinalTimeSociales().next(this.finalTime);
    }
  }
  ...
  nextQuestion() {
    if (this.currentQuestionIndex < this.levels[this.currentLevelIndex].length - 1) {
      ...
      if (this.allQuestionsAnswered()) {
        ...
        this.resetGameButtonSocialesService.setShowResetButtonSociales(true);
        this.stopTimer();
        this.storeFinalTime();
      }
    }
  ...
  onOptionSelected(levelIndex: number, questionIndex: number, option: string) {
    ...
    this.progressSocialesService.recordAnswerSociales(isCorrect);
  ...
  startTimer() {
    this.stopTimer();
    const storedClock = parseInt(localStorage.getItem('clockSociales') || '0', 10);
    this.clock = storedClock;
    this.startTime = Date.now();
    localStorage.setItem('startTimeSociales', this.startTime.toString());
    this.timerSubscription = timer(0, 1000).subscribe(() => {
      const currentTime = Date.now();
      this.clock = storedClock + Math.floor((currentTime - this.startTime) / 1000);
      localStorage.setItem('clockSociales', this.clock.toString());
    });
  }
  ...
  stopTimer() {
    ...
    localStorage.setItem('clockSociales', this.clock.toString());
    localStorage.removeItem('startTimeSociales');
  }
  ...
  resetGame() {
    ...
    if(this.allLevelsCompleted()) {
      return this.resetGameButtonSocialesService.setShowResetButtonSociales(true);
    }
  ...
  resetLevel(levelIndex: number) {
    ...
    this.progressSocialesService.removeAnswersForLevelSociales(levelIndex);
  ...
  isLevelCompleted(levelIndex: number): boolean {
    ...
    this.levelSocialesService.levelCompletedSocialesService(levelIndex);
    return true;
  }
  ...
  private checkAllLevelsCompleted() {
    const allLevelsCompleted = this.allLevelsCompleted();
    this.resetGameButtonSocialesService.setShowResetButtonSociales(allLevelsCompleted);
  ...
  saveSelectedOptions() {
    localStorage.setItem('selectedOptionsSociales', JSON.stringify(this.selectedOptions));
  }

  // cargamos las preguntas que seleccionamos
  loadSelectedOptions() {
    const storedOptions = localStorage.getItem('selectedOptionsSociales');
    if (storedOptions) {
  ...
  private restoreSelections() {
    const storedSelections = localStorage.getItem('selectedOptionsSociales');
    if (storedSelections) {
