...
import { InglesService } from 'src/app/core/services/common/original/ingles.service';
...
@Component({
  ...
  templateUrl: './ingles-niveles.component.html',
  styleUrls: ['./ingles-niveles.component.scss']
})
export class InglesNivelesComponent {
...
constructor(private inglesService: InglesService,
            ...
            private levelInglesService: LevelService
          ) {
            this.loadTimerState();
            }
...
loadTimerState() {
  const storedClock = localStorage.getItem('clockIngles');
  const storedFinalTime = localStorage.getItem('finalTimeIngles');
  ...
  this.resetGameButtonInglesService.resetGameIngles$.subscribe(() => {
    this.resetGame();
  });

  if (storedFinalTime && allLevelsCompleted) {
    this.clock = parseInt(storedFinalTime, 10);
    this.finalTimeInglesService.setFinalTimeIngles(this.clock);
    } else if (storedClock) {
...
if (this.allLevelsCompleted()) {
  ...
  localStorage.setItem('finalTimeIngles', this.finalTime.toString());
  // SOLUCION DEL TIMER QUE LLEGABA EN CERO
  this.finalTimeInglesService.getFinalTimeIngles().next(this.finalTime);
  }
}
...
nextQuestion() {
  if (this.currentQuestionIndex < this.levels[this.currentLevelIndex].length - 1) {
    ...
    if (this.allQuestionsAnswered()) {
      ...
      this.resetGameButtonInglesService.setShowResetButtonIngles(true);
      this.stopTimer();
      this.storeFinalTime();
    }
  }
...
onOptionSelected(levelIndex: number, questionIndex: number, option: string) {
  ...
  this.progressInglesService.recordAnswerIngles(isCorrect);
...
startTimer() {
  this.stopTimer();
  const storedClock = parseInt(localStorage.getItem('clockIngles') || '0', 10);
  this.clock = storedClock;
  this.startTime = Date.now();
  localStorage.setItem('startTimeIngles', this.startTime.toString());
  this.timerSubscription = timer(0, 1000).subscribe(() => {
    const currentTime = Date.now();
    this.clock = storedClock + Math.floor((currentTime - this.startTime) / 1000);
    localStorage.setItem('clockIngles', this.clock.toString());
  });
}
...
stopTimer() {
  ...
  localStorage.setItem('clockIngles', this.clock.toString());
  localStorage.removeItem('startTimeIngles');
}
...
resetGame() {
  ...
  if(this.allLevelsCompleted()) {
    return this.resetGameButtonInglesService.setShowResetButtonIngles(true);
  }
...
resetLevel(levelIndex: number) {
  ...
  this.progressInglesService.removeAnswersForLevelIngles(levelIndex);
...
isLevelCompleted(levelIndex: number): boolean {
  ...
  this.levelInglesService.levelCompletedInglesService(levelIndex);
  return true;
}
...
private checkAllLevelsCompleted() {
  const allLevelsCompleted = this.allLevelsCompleted();
  this.resetGameButtonInglesService.setShowResetButtonIngles(allLevelsCompleted);
...
saveSelectedOptions() {
  localStorage.setItem('selectedOptionsIngles', JSON.stringify(this.selectedOptions));
}

// cargamos las preguntas que seleccionamos
loadSelectedOptions() {
  const storedOptions = localStorage.getItem('selectedOptionsIngles');
  if (storedOptions) {
...
private restoreSelections() {
  const storedSelections = localStorage.getItem('selectedOptionsIngles');
  if (storedSelections) {
